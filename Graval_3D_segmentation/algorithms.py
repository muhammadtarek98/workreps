...
def sand_filter_gpu(pcd,
                    file_name: str,
                    main_dir: str = "/home/cplus/projects/m.tarek_master/graval_detection_3D/point_cloud_segmentation/cloud_scaled_full_ascii/") -> None:
    ...
    o3d.io.write_point_cloud(
        filename=f"{main_dir}{file_name}/label_1.ply",
        pointcloud=pcd_label_1,
        write_ascii=True,
        print_progress=True)
    o3d.visualization.draw_geometries([pcd_label_0])
    o3d.io.write_point_cloud(
        filename=f"/home/cplus/projects/m.tarek_master/graval_detection_3D/point_cloud_segmentation/cloud_scaled_full_ascii/{file_name}/label_0.ply",
...
def sand_fliter(pcd, file_name: str = None):
    ...
    o3d.visualization.draw_geometries([pcd_label_1])
    o3d.io.write_point_cloud(
        filename=f"/home/cplus/projects/m.tarek_master/graval_detection_3D/point_cloud_segmentation/cloud_scaled_full_ascii/{file_name}/label_1.ply",
        pointcloud=pcd_label_1,
        write_ascii=True,
        print_progress=True)
    o3d.visualization.draw_geometries([pcd_label_0])
...
def fit_ellipsoid_from_sphere(points, axes, center):
    ...
    sphere.scale(scale_factor[0] * scale_factor[1] * scale_factor[2], center=center)
    return sphere


def calculate_volume_using_fitted_ellipse_opencv(poly):
...
def g3_point_cloud_cluster(point_cloud: np.ndarray, KNN: int) -> dict:
    ...
    neighbors_distance, neighbors_indexes = tree.query(k=KNN + 1, x=xyz_detrended)
    # exculd the top node from the stack
    ...
    pcd_detrended.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamKNN(KNN))
...
def kmeans_clustering_ellipsoids_cpu(pcd):
...
def calculate_convex_hull_volume_open3d(pcd):
    convex_hull, _ = pcd.compute_convex_hull()
    volume = convex_hull.get_oriented_bounding_box().volume()
    return volume
...
def distance_function(params, cloud_points):
    ...
    distances = np.sqrt(((cloud_points[:, 0] - x_c) / a) ** 2 + ((cloud_points[:, 1] - y_c) / b) ** 2 + (
            (cloud_points[:, 2] - z_c) / c) ** 2) - 1
    return np.sum(distances ** 2)


def kmeans_cluster_gpu(pcd, number_of_clusters):
...
def kmeans_clustering_torch(pcd, device='cuda'):
    ...
    sub_sampled_pcd_np = pcd_np[subsample_indices]
    pcd_torch = torch.from_numpy(sub_sampled_pcd_np).float().to(device)
    pcd_torch = pcd_torch.unsqueeze(dim=0)
    print(pcd_torch.shape)
    kmeans = TorchKMeans(n_clusters=15000, max_iter=2, device=device)
