...
class CustomDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_dir = os.path.join(self.images_dir, self.df["images_dir"].iloc[index])
        mask_dir = os.path.join(self.masks_dir, self.df["masks_dir"].iloc[index])
        json_dir=os.path.join(self.json_dirs,self.df["json_dir"].iloc[index])
        json_data=CustomDataSet.open_json_file(file_dir=json_dir)
        image=read_image(path=image_dir)
        mask=read_image(path=mask_dir)
        objects_id=torch.unique(mask)
        objects_id=objects_id[1:]
        number_of_objects=len(objects_id)
        masks = (mask == objects_id[:, None, None]).to(dtype=torch.uint8)
        boxes=masks_to_boxes(masks=masks)
        labels=torch.ones(size=(number_of_objects,),dtype=torch.int64)
        #image_id=index
        target=dict()
