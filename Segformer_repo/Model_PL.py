...
class SegformerFinetuner(pl.LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        ...
        if self.current_epoch == 0:
            self.logger.experiment.add_graph(self.model_torch_class,
                                             images)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW([p for p in self.parameters() if p.requires_grad], lr=self.lr, eps=1e-09)
        lr_scheduler = {
            'scheduler': torch.optim.lr_scheduler.LambdaLR(optimizer=optimizer,
                                                           lr_lambda=lambda epoch: 0.95 ** self.current_epoch),
            'name': 'learning_rate'
        }
