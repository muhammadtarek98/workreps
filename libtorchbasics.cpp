...
void slicing_tensor() {
	vector<int64_t> v = { 1,2,3,4,5,6,7,8,9 };
	///reshape vector from 1d to 2d
	torch::Tensor t = torch::from_blob(v.data(), {3,3}, torch::kInt64);
	cout << t;
	// slice adn get the last column t[:,2]
	cout << t.index({ Slice(),2}) << "\n";
	...
	// t[:2,:]
	cout << t.index({ Slice(None,2),Slice() }) << endl;
	// t[:,1:]
	cout << t.index({ Slice(),Slice(1,None) }) << endl;
